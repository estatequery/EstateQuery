...
This document provides guidelines for internationalization of the EstateQuery Core software.
...
This adds `<numerusform>`s to the respective `.ts` file, which can be translated separately depending on the language. In English, this is simply:
    ...
    <message numerus="yes">
        ...
        <translation>
            <numerusform>%n active connection to EstateQuery network</numerusform>
            <numerusform>%n active connections to EstateQuery network</numerusform>
        </translation>
    </message>
...
This can be checked by executing `make translate` in the `src` directory, then verifying that `estatequery_en.ts` remains unchanged.
