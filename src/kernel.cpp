...
bool ContextualCheckZerocoinStake(int nPreviousBlockHeight, CStakeInput* stake)
{
	if (nPreviousBlockHeight < Params().Zerocoin_Block_V2_Start())
		return error("%s: zEQC stake block is less than allowed start height", __func__);

	if (CZEqcStake* zEQC = dynamic_cast<CZEqcStake*>(stake)) {
		CBlockIndex* pindexFrom = zEQC->GetIndexFrom();
		if (!pindexFrom)
			return error("%s: failed to get index associated with zEQC stake checksum", __func__);

		if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
			return error("%s: zEQC stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
		...
		uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zEQC->GetValue()));
		if (nChecksum200 != zEQC->GetChecksum())
			return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zEQC->GetChecksum(), nChecksum200);
	}
	else {
...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight)
{
	...
	if (txin.IsZerocoinSpend()) {
		...
		stake = std::unique_ptr<CStakeInput>(new CZEqcStake(spend));

		if (!ContextualCheckZerocoinStake(nPreviousBlockHeight, stake.get()))
			return error("%s: staked zEQC fails context checks", __func__);
	}
	else {
		...
		CEqcStake* eqcInput = new CEqcStake();
		eqcInput->SetInput(txPrev, txin.prevout.n);
		stake = std::unique_ptr<CStakeInput>(eqcInput);
	}
	...
	if (!txin.IsZerocoinSpend() && nPreviousBlockHeight >= Params().Zerocoin_Block_Public_Spend_Enabled() - 1) { //Equivalent for zEQC is checked above in ContextualCheckZerocoinStake()
