...
#include "lightzeqcthread.h"
#include "main.h"

/****** Thread ********/
void CLightWorker::ThreadLightZEQCSimplified() {
    RenameThread("estatequery-light-thread");
    ...
    while (true) {
        try {
            ...
            LogPrintf("%s pop work for %s \n\n", "estatequery-light-thread", genWit.toString());
            ...
            if (!pIndex) {
                ...
                if (blockHeight >= Params().Zerocoin_Block_V2_Start()) {
                    ...
                    try {
                        ...
                        LogPrintStr(std::string("ThreadLightZEQCSimplified: ") + e.message + "\n");
                    ...
                    if (!res) {
                        ...
                        ss << heightStop;
                        if (genWit.getPfrom()) {
                            LogPrintf("%s pushing message to %s \n", "estatequery-light-thread", genWit.getPfrom()->addrName);
            ...
            PrintExceptionContinue(&e, "lightzeqcthread");
            break;
        }
    }
...
void CLightWorker::rejectWork(CGenWit& wit, int blockHeight, uint32_t errorNumber) {
    if (wit.getStartingHeight() == blockHeight){
        LogPrintf("%s rejecting work %s , error code: %s\n", "estatequery-light-thread", wit.toString(), errorNumber);
        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
