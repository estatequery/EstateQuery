...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

	{
		...
		for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
			...
			for (const CTxIn& txin : tx.vin) {
				...
				if (hasZerocoinSpends) {
					...
					//Priority = (age^6+100000)*amount - gives higher priority to zeqcs that have been in mempool long
					//and higher priority to zeqcs that are large in value
					...
					// zEQC spends can have very large priority, use non-overflowing safe functions
		...
		while (!vecPriority.empty()) {
			...
			// double check that there are no double spent zEQC spends in this block or tx
			if (tx.HasZerocoinSpendInputs()) {
				...
				//This zEQC serial has already been included in the block, do not add this tx.
				if (fDoubleSerial)
					continue;
			}
		...
		if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != hashBlockLastAccumulated) {
			//For the period before v2 activation, zEQC will be disabled and previous block's checkpoint is all that will be needed
		...
		if (fProofOfStake) {
			...
			if (pblock->IsZerocoinStake()) {
				...
				if (!pwallet->GetZerocoinKey(bnSerial, key)) {
					LogPrintf("%s: failed to find zEQC with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
					return NULL;
				}

				//Sign block with the zEQC key
				if (!SignBlockWithKey(*pblock, key)) {
					LogPrintf("BitcoinMiner(): Signing new block with zEQC key failed \n");
					return NULL;
				}
			}
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
	...
	// Found a solution
	{
		...
		if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
			return error("ESTATEQUERYMiner : generated block is stale");
	}
	...
	if (!ProcessNewBlock(state, NULL, pblock)) {
		if (pblock->IsZerocoinStake()) {
			pwalletMain->zeqcTracker->RemovePending(pblock->vtx[1].GetHash());
			pwalletMain->zeqcTracker->ListMints(true, true, true); //update the state
		}
		return error("ESTATEQUERYMiner : ProcessNewBlock, block not accepted");
	}
...
{
	LogPrintf("ESTATEQUERYMiner started\n");
	SetThreadPriority(THREAD_PRIORITY_LOWEST);
	RenameThread("estatequery-miner");
	...
	while (fGenerateBitcoins || fProofOfStake) {
		...
		LogPrintf("Running ESTATEQUERYMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
