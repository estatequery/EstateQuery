...
/**
 * EstateQuery RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZeqcSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzeqcseed(const UniValue& params, bool fHelp);
extern UniValue getzeqcseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzeqc(const UniValue& params, bool fHelp);
extern UniValue dzeqcstate(const UniValue& params, bool fHelp);
