...
#include "config/estatequery-config.h"
#endif
...
void PrepareShutdown()
{
	...
	RenameThread("estatequery-shutoff");
...
void Shutdown()
{
	...
	if (nLocalServices == NODE_BLOOM_LIGHT_ZC) {
		lightWorker.StopLightZeqcThread();
	}
#ifdef ENABLE_WALLET
	delete pwalletMain;
	pwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
	...
	strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "estatequery.conf"));
...
#ifndef WIN32
	strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "estatequeryd.pid"));
#endif
	strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
	strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
	strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the EQC and zEQC money supply statistics") + " " + _("on startup"));
	...
	strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 11711, 51474));
	...
	if (GetBoolArg("-help-debug", false))
		strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in EQC/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
			FormatMoney(CWallet::minTxFee.GetFeePerK())));
	strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in EQC/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
	...
	string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, estatequery, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero, precompute, staking)"; // Don't translate these and qt below
	if (mode == HMM_BITCOIN_QT)
		debugCategories += ", qt";
	...
	strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in EQC/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
	...
	strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all EstateQuery specific functionality (Masternodes, SwiftX) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
	strUsage += HelpMessageGroup(_("Staking options:"));
	strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
	strUsage += HelpMessageOpt("-eqcstake=<n>", strprintf(_("Enable or disable staking functionality for EQC inputs (0-1, default: %u)"), 1));
	...
	strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:11711"));
	...
	strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 11710, 51475));
...
std::string LicenseInfo()
{
	return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
		...
		"\n" +
		FormatParagraph(strprintf(_("Copyright (C) 2015-%i The EstateQuery Core Developers"), COPYRIGHT_YEAR)) + "\n" +
		"\n" +
...
{
	RenameThread("estatequery-loadblk");
...
/** Sanity checks
 *  Ensure that EstateQuery is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
	...
	// Sanity check
	if (!InitSanityCheck())
		return InitError(_("Initialization sanity check failed. EstateQuery Core is shutting down."));
...
#endif
	// Make sure only a single EstateQuery process is using the data directory.
	...
	if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
		return InitError(strprintf(_("Cannot obtain a lock on data directory %s. EstateQuery Core is probably already running."), strDataDir));
	...
	if (GetBoolArg("-shrinkdebugfile", !fDebug))
		ShrinkDebugFile();
	LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
	LogPrintf("EstateQuery version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
	...
	while (!fLoaded) {
		...
		do {
			try {
				...
				//EstateQuery specific: zerocoin and spork DB's
				...
				// ESTATEQUERY: load previous sessions sporks if we have them.
				...
				if (GetBoolArg("-reindexmoneysupply", false)) {
					RecalculateZEQCMinted();
					RecalculateZEQCSpent();
					RecalculateEQCSupply(1);
				}
	...
	if (fDisableWallet) {
		...
		if (nLoadWalletRet != DB_LOAD_OK) {
			if (nLoadWalletRet == DB_CORRUPT)
				...
				strErrors << _("Error loading wallet.dat: Wallet requires newer version of EstateQuery Core") << "\n";
			else if (nLoadWalletRet == DB_NEED_REWRITE) {
				strErrors << _("Wallet needed to be rewritten: restart EstateQuery Core to complete") << "\n";
		...
		zwalletMain = new CzEQCWallet(pwalletMain->strWalletFile);
		...
		//Inititalize zEQCWallet
		uiInterface.InitMessage(_("Syncing zEQC wallet..."));

		pwalletMain->InitAutoConvertAddresses();

		bool fEnableZEqcBackups = GetBoolArg("-backupzeqc", true);
		pwalletMain->setZEqcAutoBackups(fEnableZEqcBackups);

		//Load zerocoin mint hashes to memory
		pwalletMain->zeqcTracker->Init();
	...
	/* Denominations
	   ...
	   For example:
	   1EQC+1000 == (.1EQC+100)*10
	   10EQC+10000 == (1EQC+1000)*10
	*/
	...
	if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
		...
		lightWorker.StartLightZeqcThread(threadGroup);
	}
	...
	if (pwalletMain) {
		...
		if (GetBoolArg("-precompute", false)) {
			// Run a thread to precompute any zEQC spends
