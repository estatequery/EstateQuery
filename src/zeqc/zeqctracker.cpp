...
CzEQCTracker::CzEQCTracker(std::string strWalletFile)
{
...
CzEQCTracker::~CzEQCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzEQCTracker::Init()
{
...
bool CzEQCTracker::Archive(CMintMeta& meta)
{
...
bool CzEQCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzEQCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzEQCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzEQCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzEQCTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzEQCTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzEQCTracker::GetSerialHashes()
{
...
CAmount CzEQCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzEQCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzEQCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzEQCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzEQCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzEQCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzEQCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzEQCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzEQCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzEQCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzEQCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzEQCWallet* zEQCWallet)
{
    bool iszEQCWalletInitialized = (NULL != zEQCWallet);
    CMintMeta meta;
    ...
    if (! iszEQCWalletInitialized)
        zEQCWallet = new CzEQCWallet(strWalletFile);
    meta.isSeedCorrect = zEQCWallet->CheckSeed(dMint);
    if (! iszEQCWalletInitialized)
        delete zEQCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzEQCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzEQCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzEQCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzEQCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzEQCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzEQCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzEQCWallet* zEQCWallet = new CzEQCWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zEQCWallet);
        }
        delete zEQCWallet;
        LogPrint("zero", "%s: added %d dzeqc from DB\n", __func__, listDeterministicDB.size());
    }
