...
CzEQCWallet::CzEQCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zeqc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZEQCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZEQCSeed_deprecated()) {
                    LogPrintf("%s: Updated zEQC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zeqc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zeqc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzEQCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZEQCCount(nCountLastUsed);
    else if (!walletdb.ReadZEQCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzEQCWallet::Lock()
{
    seedMaster = 0;
}

void CzEQCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzEQCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZEQC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzEQCWallet::LoadMintPoolFromDB()
{
...
void CzEQCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzEQCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzEQCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zeqcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzEQCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZEQC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zeqcTracker which also adds to database
    pwalletMain->zeqcTracker->Add(dMint, true);
...
void CzEQCWallet::SeedToZEQC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzEQCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzEQCWallet::GenerateDeterministicZEQC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzEQCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzEQCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzEQCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
