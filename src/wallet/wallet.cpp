...
#include "zeqc/zeqcwallet.h"
#include "zeqc/zeqctracker.h"
...
/**
 * Fees smaller than this (in ueqc) are considered zero fee (for transaction creation)
...
{
    return zeqcTracker->HasSerial(bnSerial);
}
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate)
{
    ...
    bool fCheckZEQC = GetBoolArg("-zapwallettxes", false);
    if (fCheckZEQC)
        zeqcTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            //If this is a zapwallettx, need to readd zeqc
            if (fCheckZEQC) {
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zEQC in the future, for now it's useless,
        ...
        CAmount nBalance = 0;
        vector<CMintMeta> vMints = zeqcTracker->GetMints(true);
    ...
    return zeqcTracker->GetBalance(false, false);
}
...
{
    return zeqcTracker->GetUnconfirmedBalance();
}
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        set<CMintMeta> setMints = zeqcTracker->ListMints(true, true, true);
...
bool CWallet::SelectStakeCoins(std::list<std::unique_ptr<CStakeInput> >& listInputs, CAmount nTargetAmount, bool fPrecompute)
{
    ...
    if (GetBoolArg("-eqcstake", true) && !fPrecompute) {
        for (const COutput &out : vCoins) {
            ...
            std::unique_ptr<CEqcStake> input(new CEqcStake());
            input->SetInput((CTransaction) *out.tx, out.i);
            listInputs.emplace_back(std::move(input));
        }
    }

    /* Disable zEQC Staking
    //zEQC
    if ((GetBoolArg("-zeqcstake", true) || fPrecompute) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() && !IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        //Only update zEQC set once per update interval
        bool fUpdate = false;
        ...
        set<CMintMeta> setMints = zeqcTracker->ListMints(true, true, fUpdate);
        for (auto meta : setMints) {
            ...
            if (meta.nHeight < chainActive.Height() - Params().Zerocoin_RequiredStakeDepth()) {
                std::unique_ptr<CZEqcStake> input(new CZEqcStake(meta.denom, meta.hashStake));
                listInputs.emplace_back(std::move(input));
            }
        }
    }
...
bool CWallet::MintableCoins()
{
    ...
    CAmount nZeqcBalance = GetZerocoinBalance(false);
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 EQC collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    vector<pair<CScript, CAmount> > vecSend;
    vecSend.push_back(make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 EQC collateral
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 EQC.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 10000 EQC.");
                    } else {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-estatequery-address
                    CScript scriptChange;
        ...
        )
{
    ...
    for (std::unique_ptr<CStakeInput>& stakeInput : listInputs) {
        ...
        if (Stake(stakeInput.get(), nBits, block.GetBlockTime(), nTxNewTime, hashProofOfStake)) {
            ...
            //Masternode payment
            FillBlockPayee(txNew, nMinFee, true, stakeInput->IsZEQC());
            ...
            //Mark mints as spent
            if (stakeInput->IsZEQC()) {
                CZEqcStake* z = (CZEqcStake*)stakeInput.get();
...
void CWallet::CreateAutoMintTransaction(const CAmount& nMintAmount, CCoinControl* coinControl)
{
    if (nMintAmount > 0){
        ...
        LogPrintf("CWallet::AutoZeromint() @ block %ld: successfully minted %ld zEQC. Current percentage of zEQC: %lf%%\n",
...
void CWallet::AutoZeromint()
{
    ...
    // zEQC are integers > 0, so we can't mint 10% of 9 EQC
    if (nBalance < 10){
        LogPrint("zero", "CWallet::AutoZeromint(): available balance (%ld) too small for minting zEQC\n", nBalance);
        return;
    }

    // Percentage of zEQC we already have
    ...
    if(dPercentage >= nZeromintPercentage){
        LogPrint("zero", "CWallet::AutoZeromint() @block %ld: percentage of existing zEQC (%lf%%) already >= configured percentage (%d%%). No minting needed...\n",
                  chainActive.Tip()->nHeight, dPercentage, nZeromintPercentage);
        return;
    }

    // zEQC amount needed for the target percentage
    nToMintAmount = ((nZerocoinBalance + nBalance) * nZeromintPercentage / 100);

    // zEQC amount missing from target (must be minted)
    nToMintAmount = (nToMintAmount - nZerocoinBalance) / COIN;

    // Use the biggest denomination smaller than the needed zEQC We'll only mint exact denomination to make minting faster.
    ...
    // If a preferred denomination is used (means nPreferredDenom != 0) do nothing until we have enough EQC to mint this denomination
...
bool CWallet::CreateZEQCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwalletMain->GenerateDeterministicZEQC(denomination, coin, dMint);
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, vector<CDeterministicMint>& vDMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    while (true) {
        ...
        if (!CreateZEQCOutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new zeqc output", __func__);
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to Eqc
    ...
    // Sign if these are estatequery outputs - NOTE that zEQC outputs are signed later in SoK
    if (!isZCSpendChange) {
        int nIn = 0;
        for (const std::pair<const CWalletTx*, unsigned int>& coin : setCoins) {
            if (!SignSignature(*this, *coin.first, txNew, nIn++)) {
...
bool CWallet::CheckCoinSpend(libzerocoin::CoinSpend& spend, libzerocoin::Accumulator& accumulator, CZerocoinSpendReceipt& receipt)
{
    if (!spend.Verify(accumulator)) {
        receipt.SetStatus(_("The transaction did not verify"), ZEQC_BAD_SERIALIZATION);
    ...
    if (Params().NetworkID() != CBaseChainParams::REGTEST && IsSerialKnown(spend.getCoinSerialNumber())) {
        //Tried to spend an already spent zEQC
        receipt.SetStatus(_("The coin spend has been used"), ZEQC_SPENT_USED_ZEQC);
        uint256 hashSerial = GetSerialHash(spend.getCoinSerialNumber());
        if(!zeqcTracker->HasSerialHash(hashSerial))
            return error("%s: serialhash %s not found in tracker", __func__, hashSerial.GetHex());

        CMintMeta meta = zeqcTracker->Get(hashSerial);
        meta.isUsed = true;
        if (!zeqcTracker->UpdateState(meta))
                         ...
                         CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    receipt.SetStatus(_("Transaction Mint Started"), ZEQC_TXMINT_GENERAL);
    ...
    while (nLockAttempts < 100) {
        TRY_LOCK(zeqcTracker->cs_spendcache, lockSpendcache);
        if (!lockSpendcache) {
        ...
        for (auto &it : mapMintsSelected) {
            ...
            CMintMeta meta = zeqcTracker->Get(GetSerialHash(mint.GetSerialNumber()));
            CoinWitnessData *coinWitness = zeqcTracker->GetSpendCache(meta.hashStake);
            ...
            if (!GenerateAccumulatorWitness(coinWitness, mapAccumulators, pindexCheckpoint)) {
                receipt.SetStatus(_("Couldn't generate the accumulator witness"),
                                  ZEQC_FAILED_ACCUMULATOR_INITIALIZATION);
           ...
           /* if (nVersion >= libzerocoin::PrivateCoin::PUBKEY_VERSION) {*/
                ...
                if (!mint.GetKeyPair(key))
                    return error("%s: failed to set zEQC privkey mint version=%d", __func__, nVersion);
            ...
            try {
                ...
                if (!CheckCoinSpend(spend, accumulator, receipt)) {
                    receipt.SetStatus(_("CoinSpend: failed check"), ZEQC_SPEND_ERROR);
                    return error("%s : %s", __func__, receipt.GetStatusMessage());
                }
                ...
                receipt.SetStatus(_("CoinSpend: Accumulator witness does not verify"), ZEQC_INVALID_WITNESS);
                return error("%s : %s", __func__, receipt.GetStatusMessage());
            }
        }
        break;
    ...
    if (nLockAttempts == 100) {
        ...
        receipt.SetStatus(_("could not get lock on cs_spendcache"), ZEQC_TXMINT_GENERAL);
        return false;
    }
    ...
    receipt.SetStatus(_("Spend Valid"), ZEQC_SPEND_OKAY); // Everything okay
    ...
    while (nLockAttempts < 100) {
        ...
        for (auto &it : mapMintsSelected) {
            ...
            if (!GetTransaction(mint.GetTxHash(), txMint, hashBlock)) {
                receipt.SetStatus(strprintf(_("Unable to find transaction containing mint %s"), mint.GetTxHash().GetHex()), ZEQC_TXMINT_GENERAL);
                return false;
            } else if (mapBlockIndex.count(hashBlock) < 1) {
                // check that this mint made it into the blockchain
                receipt.SetStatus(_("Mint did not make it into blockchain"), ZEQC_TXMINT_GENERAL);
                return false;
            }
            ...
            if (outputIndex == -1) {
                receipt.SetStatus(_("Pubcoin not found in mint tx"), ZEQC_TXMINT_GENERAL);
                return false;
            }

            mint.SetOutputIndex(outputIndex);
            CTxIn in;
            if(!ZEQCModule::createInput(in, mint, hashTxOut)){
                receipt.SetStatus(_("Cannot create public spend input"), ZEQC_TXMINT_GENERAL);
                return false;
            }
            vin.emplace_back(in);
        ...
        bool isPublicSpend)
{
    ...
    int nStatus = ZEQC_TRX_FUNDS_PROBLEMS;
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zEQC"), nStatus);
        return false;
    }

    // Create transaction
    nStatus = ZEQC_TRX_CREATE;
    ...
    const int nMaxSpends = Params().Zerocoin_MaxPublicSpendsPerTransaction(); // Maximum possible spends for one zEQC public spend transaction
    vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the zEQC used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
        setMints = zeqcTracker->ListMints(true, false, true, true); // need to find mints to spend
        if(setMints.empty()) {
        ...
        // Select the zEQC mints to use in this spend
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            //add output to estatequery address to the transaction (the actual primary spend taking place)
            ...
            if (nBytes >= MAX_ZEROCOIN_TX_SIZE) {
                receipt.SetStatus(_("In rare cases, a spend with 7 coins exceeds our maximum allowable transaction size, please retry spend using 6 or less coins"), ZEQC_TX_TOO_LARGE);
                return false;
            }
    ...
    receipt.SetStatus(_("Transaction Created"), ZEQC_SPEND_OKAY); // Everything okay
...
void CWallet::ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored, std::list<CDeterministicMint>& listDMintsRestored)
{
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!zeqcTracker->UnArchive(hashPubcoin, false)) {
            LogPrintf("%s : failed to unarchive mint %s\n", __func__, mint.GetValue().GetHex());
        } else {
            zeqcTracker->UpdateZerocoinMint(mint);
        }
    ...
    for (CDeterministicMint dMint : listDMints) {
        ...
        if (!zeqcTracker->UnArchive(dMint.GetPubcoinHash(), true)) {
            LogPrintf("%s : failed to unarchive deterministic mint %s\n", __func__, dMint.GetPubcoinHash().GetHex());
        } else {
            zeqcTracker->Add(dMint, true);
        }
        listDMintsRestored.emplace_back(dMint);
    }
}

string CWallet::GetUniqueWalletBackupName(bool fzeqcAuto) const
{
    ...
    return strprintf("wallet%s.dat%s", fzeqcAuto ? "-autozeqcbackup" : "", DateTimeStrFormat(".%Y-%m-%d-%H-%M", GetTime()));
}

void CWallet::ZEqcBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        strNewBackupName = strprintf("wallet-autozeqcbackup-%d.dat", i);
        ...
        if (filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozeqcbackup-%d.dat", i + 1);
            ...
            strNewBackupName = strprintf("wallet-autozeqcbackup-%d.dat", 0);
            backupPath = backupDir / strNewBackupName;
            break;
        }
    ...
    if(!GetArg("-zeqcbackuppath", "").empty()) {
        filesystem::path customPath(GetArg("-zeqcbackuppath", ""));
...
bool CWallet::SpendZerocoin(CAmount nAmount, CWalletTx& wtxNew, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vMintsSelected, bool fMintChange, bool fMinimizeChange, CBitcoinAddress* addressTo, bool isPublicSpend)
{
    ...
    int nStatus = ZEQC_SPEND_ERROR;

    if (IsLocked()) {
        receipt.SetStatus("Error: Wallet locked, unable to create transaction!", ZEQC_WALLET_LOCKED);
        return false;
    }
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        nStatus = ZEQC_COMMIT_FAILED;

        //reset all mints
        for (CZerocoinMint mint : vMintsSelected) {
            uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
            zeqcTracker->SetPubcoinNotUsed(hashPubcoin);
        ...
        for (CZerocoinSpend spend : receipt.GetSpends()) {
            if (!walletdb.EraseZerocoinSpendSerialEntry(spend.GetSerial())) {
                receipt.SetStatus("Error: It cannot delete coin serial number in wallet", ZEQC_ERASE_SPENDS_FAILED);
            }
        ...
        for (auto& dMint : vNewMints) {
            if (!walletdb.EraseDeterministicMint(dMint.GetPubcoinHash())) {
                receipt.SetStatus("Error: Unable to cannot delete zerocoin mint in wallet", ZEQC_ERASE_NEW_MINTS_FAILED);
            }
        }
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        zeqcTracker->SetPubcoinUsed(hashPubcoin, txidSpend);

        CMintMeta metaCheck = zeqcTracker->GetMetaFromPubcoin(hashPubcoin);
        if (!metaCheck.isUsed) {
    ...
    receipt.SetStatus("Spend Successful", ZEQC_SPEND_OKAY);  // When we reach this point spending zEQC was successful
...
{
    CMintMeta meta;
    if (!zeqcTracker->GetMetaFromStakeHash(hashStake, meta))
...
{
    if (zeqcTracker->HasPubcoin(bnValue))
        return true;
...
bool CWallet::UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CZerocoinMint mint;
    if (zeqcTracker->HasPubcoinHash(hashValue)) {
        CMintMeta meta = zeqcTracker->GetMetaFromPubcoin(hashValue);
        meta.nHeight = nHeight;
        meta.txid = txid;
        return zeqcTracker->UpdateState(meta);
    } else {
...
void CWallet::PrecomputeSpends()
{
    ...
    RenameThread("estatequery-precomputer");
    ...
    while (true) {
        ...
        for (std::unique_ptr <CStakeInput>& stakeInput : listInputs) {
...

            {
                ...
                CoinWitnessData* witnessData = zeqcTracker->GetSpendCache(serialHash);
