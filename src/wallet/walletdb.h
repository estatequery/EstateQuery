...
public:
    ...
    bool WriteZEQCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZEQCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZEQCSeed_deprecated(uint256& seed);
    bool EraseZEQCSeed();
    bool EraseZEQCSeed_deprecated();

    bool WriteZEQCCount(const uint32_t& nCount);
    bool ReadZEQCCount(uint32_t& nCount);
