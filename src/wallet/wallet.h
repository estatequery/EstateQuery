...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zeqcspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 EQC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zEQC send
enum ZerocoinSpendStatus {
    ZEQC_SPEND_OKAY = 0,                            // No error
    ZEQC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZEQC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZEQC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZEQC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZEQC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZEQC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZEQC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZEQC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZEQC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZEQC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZEQC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZEQC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZEQC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZEQC_SPENT_USED_ZEQC = 14,                      // Coin has already been spend
    ZEQC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZEQC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZEqcBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZEQCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzeqcAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzEQCTracker> zeqcTracker;
    ...
    void setZWallet(CzEQCWallet* zwallet)
    {
        zwalletMain = zwallet;
        zeqcTracker = std::unique_ptr<CzEQCTracker>(new CzEQCTracker(strWalletFile));
    }

    CzEQCWallet* getZWallet() { return zwalletMain; }
    ...
    void setZEqcAutoBackups(bool fEnabled)
    {
    ...
    /** zEQC reset */
    boost::signals2::signal<void()> NotifyzEQCReset;
