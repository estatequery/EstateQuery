...
#include "test/test_estatequery.h"
...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zeqcTracker = unique_ptr<CzEQCTracker>(new CzEQCTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZEQC_TRX_FUNDS_PROBLEMS, strprintf("Failed Invalid Amount Check: %s", receipt.GetStatusMessage()));
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZEQC_TRX_FUNDS_PROBLEMS, strprintf("Failed Invalid Amount Check: %s", receipt.GetStatusMessage()));
...
BOOST_AUTO_TEST_CASE(zerocoin_public_spend_test)
{
    ...
    CTxIn in;
    if (!ZEQCModule::createInput(in, mint, tx.GetHash())){
        BOOST_CHECK_MESSAGE(false, "Failed to create zc input");
    }

    PublicCoinSpend publicSpend(ZCParams);
    if (!ZEQCModule::validateInput(in, out, tx, publicSpend)){
        BOOST_CHECK_MESSAGE(false, "Failed to validate zc input");
    }

    PublicCoinSpend publicSpendTest(ZCParams);
    BOOST_CHECK_MESSAGE(ZEQCModule::parseCoinSpend(in, tx, out, publicSpendTest), "Failed to parse public spend");
    ...
    BOOST_CHECK_MESSAGE(!ZEQCModule::validateInput(in, out, tx, publicSpend2), "Different denomination");
