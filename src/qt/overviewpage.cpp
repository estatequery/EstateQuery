...
class TxViewDelegate : public QAbstractItemDelegate
{
	Q_OBJECT
public:
	TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::EQC)
	{
	}
	...
	const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
	...
	CAmount eqcAvailableBalance = balance - immatureBalance - nLockedBalance;
	CAmount nUnlockedBalance = nTotalBalance - nLockedBalance;

	// EQC Watch-Only Balance
	CAmount nTotalWatchBalance = watchOnlyBalance + watchUnconfBalance;
	CAmount nAvailableWatchBalance = watchOnlyBalance - watchImmatureBalance - nWatchOnlyLockedBalance;

	// Combined balances
	CAmount availableTotalBalance = eqcAvailableBalance;
	CAmount sumTotalBalance = nTotalBalance;

	// EQC labels
	ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, eqcAvailableBalance, false, BitcoinUnits::separatorAlways));
	...
	// EQC Available
	bool showEQCAvailable = eqcAvailableBalance != nTotalBalance;
	bool showWatchOnlyEQCAvailable = showEQCAvailable || nAvailableWatchBalance != nTotalWatchBalance;
	ui->labelBalanceText->setVisible(showEQCAvailable || showWatchOnlyEQCAvailable);
	ui->labelBalance->setVisible(showEQCAvailable || showWatchOnlyEQCAvailable);
	ui->labelWatchAvailable->setVisible(showWatchOnlyEQCAvailable && showWatchOnly);

	// EQC Pending
	bool showEQCPending = unconfirmedBalance != 0;
	bool showWatchOnlyEQCPending = showEQCPending || watchUnconfBalance != 0;
	ui->labelPendingText->setVisible(showEQCPending || showWatchOnlyEQCPending);
	ui->labelUnconfirmed->setVisible(showEQCPending || showWatchOnlyEQCPending);
	ui->labelWatchPending->setVisible(showWatchOnlyEQCPending && showWatchOnly);

	// EQC Immature
	bool showEQCImmature = immatureBalance != 0;
	bool showWatchOnlyImmature = showEQCImmature || watchImmatureBalance != 0;
	ui->labelImmatureText->setVisible(showEQCImmature || showWatchOnlyImmature);
	ui->labelImmature->setVisible(showEQCImmature || showWatchOnlyImmature); // for symmetry reasons also show immature label when the watch-only one is shown
	ui->labelWatchImmature->setVisible(showWatchOnlyImmature && showWatchOnly); // show watch-only immature balance

																				// EQC Locked
	bool showEQCLocked = nLockedBalance != 0;
	bool showWatchOnlyEQCLocked = showEQCLocked || nWatchOnlyLockedBalance != 0;
	ui->labelLockedBalanceText->setVisible(showEQCLocked || showWatchOnlyEQCLocked);
	ui->labelLockedBalance->setVisible(showEQCLocked || showWatchOnlyEQCLocked);
	ui->labelWatchLocked->setVisible(showWatchOnlyEQCLocked && showWatchOnly);
